...
namespace litsignal { namespace processing {
    vec calculate_phase_vocoder(const vec &input_signal, float alpha = 0.5f, int window_size = 2048, int syn_hop_size = -1);

    struct PhaseVocoderContext {
        PhaseVocoderContext() = default;
    };
    ...
    public:
        ...
        PhaseVocoderPipeline(RunnerType *runner, const vec &input_signal, vec &output, int window_size = 2048,
                             float alpha = 0.5f, int syn_hop_size = -1);

        PhaseVocoderPipeline(RunnerType *runner, const vec &input_signal, vec &output, const vec &window,
                             float alpha = 0.5f, int syn_hop_size = -1);

...
vec processing::detail::resample(const vec &signal, int upfactor, int downfactor) {
    const int n = 10;
    if (upfactor <= 0 || downfactor <= 0) throw std::runtime_error("factors must be positive integer");

    int gcd = lit::math::calc_gcd(upfactor, downfactor);
    upfactor /= gcd;
    downfactor /= gcd;
    ...
    int output_size = lit::math::quotient_ceil((int)signal.size() * upfactor, downfactor);
    ...
    vec resr = processing::detail::upfirdn(signal, upfactor, downfactor, h);
    return resr(span((uword)delay, (uword) delay + output_size - 1));
}

vec processing::detail::firls(int length, vec freq, const vec &amplitude) {
    ...
    for (uword i = 0; i < freq.size(); i += 2) {
        ...
        b += slope / (PI2 * PI2) * (cos(PI2 * k * freq.at(i + 1)) - cos(PI2 * k * freq.at(i))) / (k % k);
        b += (freq.at(ii) * (slope * freq.at(ii) + b1) * lit::math::sinc_fac(2 * k * freq.at(ii)))
             - (freq.at(i) * (slope * freq.at(i) + b1) * lit::math::sinc_fac(2 * k * freq.at(i)));
        b *= w_half_q;
    }
...
processing::detail::Resampler::Resampler(int up_rate, int down_rate, const vec &coefs)
...
processing::detail::Resampler::~Resampler() {
    delete [] _transposed_coefs;
    delete [] _state;
}

int processing::detail::Resampler::out_count(int in_count) {
...
vec processing::detail::Resampler::apply(vec &in) {
...
vec processing::detail::upfirdn(const vec &signal, int up_rate, int down_rate, const vec &filter) {

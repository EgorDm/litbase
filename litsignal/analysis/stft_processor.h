...
namespace litsignal { namespace analysis { namespace fourier_analysis {
    ...
    public:
        STFTProcessor(vec &input, cx_mat &output, int &feature_rate, vec &t, vec &f, int sr, const vec &window,
            ...
            if (coefficient_range.size() < 2) this->coefficient_range = span(0, ACU(frame_size / 2) - 1);
            else this->coefficient_range = span(ACU(coefficient_range.front()), ACU(coefficient_range.back()) - 1);
